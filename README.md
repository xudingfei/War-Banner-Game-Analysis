...
新增 [C++ 实现说明](Cpp/README.md)：部分原脚本规则重建、权重抽样、原创模板生成、结构验证和简化战斗模型的开局解法搜索。源码及测试可独立编译；1,000 个种子通过确定性与行动重放测试。

这里的开局零电网损失证明只适用于文档明确规定的参考模型，不代表原游戏或现有 UE 完整对局必胜。本次未修改 UE 工程。
